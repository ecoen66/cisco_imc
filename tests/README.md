...
`pytest --durations=10 --cov-report term-missing --cov=custom_components.cisco_imc tests` | This tells `pytest` that your target module to test is `custom_components.cisco_imc` so that it can give you a [code coverage](https://en.wikipedia.org/wiki/Code_coverage) summary, including % of code that was executed and the line numbers of missed executions.
